                                 ...
                                 bool         has_pusch_grant)
{
  ...
  // rbgmask_t temp_mask(dl_mask.size());
  // temp_mask.fill(0,10);
  // dl_mask &= temp_mask;
  // fmt::memory_buffer str_buffer;
  // fmt::format_to(str_buffer, fmt::format("{:b}", alloc_type));
...
alloc_result sf_grid_t::alloc_dl_ctrl(uint32_t aggr_idx, rbg_interval rbg_range, alloc_type_t alloc_type)
{
  ...
  // rbgmask_t temp_mask(new_mask.size());
  // temp_mask.fill(0,10);
  // new_mask &= temp_mask;
...
{
// // here the user mask is being modified as per our need.
//   rbgmask_t temp1_mask(user_mask.size()), temp_mask(user_mask.size());
// // We created temp1_mask and filled with 10 1s which means it is allocating 10 RGBs
//   temp1_mask.fill(0, 10);
//   temp_mask |= temp1_mask; // temp_mask is being modified with 10 1s
//
//   *(rbgmask_t*)&user_mask = temp_mask; // This changes the user mask and it will carry on through out
//
//   fmt::memory_buffer str_buffer;
//   fmt::format_to(str_buffer, fmt::format("{:b}", user_mask));
//   printf(" in sched_grid at 229 %s\n", srsran::to_c_str(str_buffer));
  ...
  //fmt::format_to(string_buffer,
  //                   "Line 216: dci={}",
  //			dci_format);
  //printf("%s \n", srsran::to_c_str(string_buffer));
  // ******************************************************************
  return ret;
}
...
{
  // code doesnt come here ... this is useless
